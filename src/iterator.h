...
typedef platform_status (*iterator_step_fn)(iterator *itor, uint32 *did_we_miss);
typedef platform_status (*iterator_seek_fn)(iterator  *itor,
                                            key        seek_key,
                                            comparison from_above,
                                            uint32 *did_we_miss);
typedef void (*iterator_print_fn)(iterator *itor, uint32 *did_we_miss);
...
iterator_next(iterator *itor, uint32 *did_we_miss)
{
   return itor->ops->next(itor, did_we_miss);
}

static inline platform_status
iterator_prev(iterator *itor, uint32 *did_we_miss)
{
   return itor->ops->prev(itor, did_we_miss);
}

static inline platform_status
iterator_seek(iterator *itor, key seek_key, comparison seek_type, uint32 *did_we_miss)
{
   return itor->ops->seek(itor, seek_key, seek_type, did_we_miss);
}

static inline void
iterator_print(iterator *itor, uint32 *did_we_miss)
{
